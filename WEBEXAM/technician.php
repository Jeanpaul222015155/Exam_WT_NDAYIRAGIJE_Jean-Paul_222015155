...
<body style="background-color: yellowgreen;">
  ...
  <section>
    <h1>Technician Registration Form</h1>
    <form method="post" onsubmit="return confirmInsert();">
        <label for="technician_id">Technician ID:</label>
        <input type="number" id="technician_id" name="technician_id" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add'])) {
    ...
    $stmt = $connection->prepare("INSERT INTO technicians (Technician_id, First_name, Last_name, Email, Phone, Specialty) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("isssss", $technician_id, $first_name, $last_name, $email, $phone, $specialty);

    if ($stmt->execute()) {
        echo "New technician record has been added successfully.<br><br><a href='technician.php'>Back to Form</a>";
    } else {
      ...
      if ($result->num_rows > 0) {
          while ($row = $result->fetch_assoc()) {
              echo "<tr>
                    ...
                    <td>{$row['Phone']}</td>
                    <td>{$row['Specialty']}</td>
                    <td><a href='delete_technician.php?technician_id={$row['Technician_id']}'>Delete</a></td>
                    <td><a href='update_technician.php?technician_id={$row['Technician_id']}'>Update</a></td>
                   </tr>";
          }
      } else {
